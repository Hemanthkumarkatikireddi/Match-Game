...
import TabItem from './Component/TabItems'

import Thumbnails from './Component/TabsList'
...
class App extends Component {
  ...
  onChangeDisplayItem = tabId => {
    this.setState({displayItem: tabId})
  }

  onChangeMatchItem = imageUrl => {
  ...
  RenderScoreCard = () => {
    ...
    return (
      <div className="scorecard-container">
        <div className="background-score-image-container">
  ...
  render() {
    const {displayItem, time, matchImage, score, view} = this.state

    const showList = imagesList.filter(each => each.category === displayItem)
    ...
    return (
      <div className="background-container">
        ...
        {view ? (
          <div>
            <div className="shown-image-container">
              <img className="shown-image" src={matchImage} alt="match" />
            </div>
            <div>
              <ul className="tab-switch">
                {tabsList.map(each => (
                  <TabItem
                    tabs={each}
                    changeItems={this.onChangeDisplayItem}
                    key={each.tabId}
                    classB={each.tabId === displayItem}
                  />
                ))}
              </ul>
            </div>
              ...
              <ul className="thumbnails-list">
                {showList.map(each => (
                  <Thumbnails
                    list={each}
                    key={each.id}
                    matchImage={this.onChangeMatchItem}
                  />
                ))}
              </ul>
            </div>
